...
namespace StudioLaValse.ScoreDocument.Reader.Extensions
{
    ...
    {
        public static IEnumerable<IPageReader> ReadPages(this IScoreDocumentReader scoreDocument, ScoreDocumentStyleTemplate styleTemplate)
        {
            var scoreDocumentLayout = scoreDocument.ReadLayout();

            var currentpage = new Page(0, styleTemplate);
            ...
            foreach (var measure in scoreDocument.ReadScoreMeasures())
            {
                ...
                var currentSystemLength = currentSystem.ScoreMeasures.Select(m => m.ReadLayout().Width).Sum();
                ...
                if (currentSystemLength > currentAvailableWidth && currentSystem.ScoreMeasures.Any())
                {
                    ...
                    if (currentSystemCanvasBottom > currentLowestAllowedPoint)
                    {
                        ...
                        currentpage = new Page(pageIndex, styleTemplate);
